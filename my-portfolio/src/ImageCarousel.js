...
export default function ImageCarousel({ images = [] }) {
  ...
  return (
    <section className="relative z-10 w-full py-20 overflow-hidden bg-gradient-to-r from-transparent via-indigo-500/5 to-transparent">
      ...
      <div className="relative w-full overflow-hidden">
        ...
        >
          {duplicatedImages.map((src, index) => (
            <motion.div
              key={index}
              className="relative flex-shrink-0 w-96 h-72 rounded-2xl overflow-hidden bg-white/5 backdrop-blur-lg border border-white/10 shadow-lg carousel-image-small hover:shadow-2xl transition-shadow duration-300"
              initial={{ rotateZ: 0 }}
              whileHover={{ rotateZ: 3, scale: 1.08 }}
              transition={{ duration: 0.3 }}
              style={{
                rotateZ: (Math.sin(index * 0.5) * 2.5),
              }}
            >
